...
impl RegistryType{
    ...
    fn parse_afd(&self, appstate: &mut AppState, linha: &str){
        match self{
            Self::Cabecalho => {
                ...
                let cabecalho = Cabecalho{
                    ...
                    data_inicio: NaiveDateTime::parse_from_str(&data_inicio, "%Y-%m-%dT%H:%M:%S").unwrap(),
                        // SelDate::new_by_str(data_inicio),
                    data_final: NaiveDateTime::parse_from_str(&data_final, "%Y-%m-%dT%H:%M:%S").unwrap(),
...
struct Cabecalho{
    ...
    razao_social: String,
    id_rep: String,
    data_inicio: NaiveDateTime,
    data_final: NaiveDateTime,
